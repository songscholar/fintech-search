...
### 2026-04-22 聊天页化后的前端接入方式

当前智能体页不再展示一整块常驻参数卡片，而是走更接近聊天产品的输入模型：
...
前端发给 `/agent/chat` 的 payload 新增支持：
...
- `attachments`
  - 图片：发送 `data_url`
  - 文本类文件：发送截断后的 `text_content`
- `provider_override`
  - 来自前端弹层配置
  - 允许用户在浏览器侧临时切换模型、Base URL、API Key

这样做之后：

- 页面本身更干净
- 模型配置仍然可调
- 后端代理层仍然存在，不会把真实上游协议直接暴露给前端

### 2026-04-22 设置菜单与布局收口

这一步没有改后端协议，但对前端交互结构做了两类很关键的修正。
...
- 点击 `设置` 只展开一个轻量菜单
- 只有继续点击具体动作，才真正打开模型弹层

同时在样式层补了：

- `modal-backdrop[hidden] { display: none !important; }`

这样可以避免“元素虽然带 hidden，但视觉上还挂在页面里”的问题。
...
- `insight-board` 改成更稳定的等高网格
- 能更完整地占满工作区，而不是只有几块卡片漂在中间

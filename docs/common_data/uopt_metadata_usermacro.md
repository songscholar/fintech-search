...
| 日期 | 版本 | 作者 | 说明 |
|------|------|------|------|
| 2024-05-13 17:34:48 | 3.0.1002.2 | wuxd | 调整获取冲销流水号宏函数hs_snprintf为snprintf |
| 2024-02-20 10:03:05 | 3.0.1002.1 | 沈勋 | 新增[获取调用功能号] |
...
| 2023-06-12 17:07 | 3.0.0.2 | 吴丽丽 | 修改微秒获取、增加获取核心分片号宏 |
...
- **参数**: `[标准字段]`
- **适用类型**: 函数,服务,原子服务,原子函数,因子服务

```
@[标准字段] =((CCustomContext *)(lpIUFTContext->GetBizContext()))->uconvert_partition_no;
```
...
ILdpMsgReader * lpMsgReader = lpIUFTContext->GetLdpMsgReader();
auto lpOptHead = lpMsgReader->GetOptionalHead();
@[标准字段1] = lpOptHead->Contains(42);
const char * lpTraceID = lpOptHead->GetStr(8);
if(lpTraceID)
{
    hs_strcpy(@[标准字段2],lpTraceID);
}
else
{
    //traceid无效
...
**说明**: 获取序列号后，拼接分片号
示例代码：[获取分片序列号][CNST_USES_SEQUENCE_ENTRUST_REPORT_NO][@serial_no]

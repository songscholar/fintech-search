# umgr - 用户自定义宏

用户自定义宏定义，包含宏名称、参数、内容和使用说明。

## 修改历史

| 日期 | 版本 | 作者 | 说明 |
|------|------|------|------|
| 2024-09-30 13:30:01 | 3.0.5.1001 | 余丰亮 | 内存交易错误号信息以及错误号二次重置代码优化 |
| 2024-08-15 17:27:44 | 3.0.3.1 | 董瑞辉 | 调整自定义宏获取额度管理分片的使用范围 |
| 2025-07-17 13:46:16 | 3.0.2.3 | 乐闽庭 | 增加[获取交易参数部署标志]、[获取交易管理部署标志]宏 |
| 2024-05-14 14:45:11 | 3.0.2.2 | 徐志坚 | 修正[clob字段解包]的错误返回方式 |
| 2024-04-16 13:44:16 | 3.0.2.1 | 徐志坚 | 将[获取冲销流水号]宏中用到的hs_snprintf改为snprintf |
| 2024-04-03 16:52:29 | V3.0.1.9 | 徐志坚 | 增加[轮询错误处理]宏 |
| 2024-01-06 11:14:21 | V3.0.1.8 | 楼欣欣 | go to end使用方式优化  |
| 2023-12-23 15:12:33 | V3.0.1.7 | 黄积冲 | 获取请求分片号、获取RPC超时时间、获取回库RPC超时时间、获取转换服务分片号允许因子服务,因子函数执行 |
| 2023-12-19 14:22:28 | V3.0.1.6 | 李海洋 | 修复业务自定义上下文二进制兼容问题  |
| 2023-11-20 10:29:03 | V3.0.1.5 | 程猛 | 调整宏[节点定位插件分片状态更新]的报错信息 |
| 2023-10-31 09:41:17 | V3.0.1.4 | 程猛 | 支持uconvert分片号从json配置获取 |
| 2023-09-25 19:19:24 | V3.0.1.3 | 华曌 | 新增获取RPC、回库RPC超时时间自定义宏 |
| 2023-09-19 11:04:17 | V3.0.1.2 | 徐志坚 | 新增[获取仲裁状态] |
| 2023-09-04 14:40:43 | V3.0.1.1 | 沈勋 | 版本统一调整 |
| 2023-08-30 13:54:17 | 3.0.0.10 | 黄佳平 | deploy_type字典变更改造 |
| 2023-08-23 19:46:05 | 3.0.0.9 | 余世泽 | 新增[业务复核标志] |
| 2023-08-21 12:28 | 3.0.0.8 | 周贺 | 新增[获取请求分片号] |
| 2023-08-08 14:38 | 3.0.0.7 | 杨森峰 | 临时修改[业务远程调用]宏 |
| 2023-07-27 15:18 | 3.0.0.6 | 吴威 | 调整自定义宏[clob字段解包]，去除[解包器]->Release(); |
| 2023-07-25 21:19 | 3.0.0.5 | 李海洋 | 不合理依赖问题修复 |

> 共 25 条修改记录，仅显示最近20条


## 宏列表（共 10 个）

### 获取交易管理分片号

- **参数**: `[标准字段]`
- **适用类型**: 函数,服务,原子服务,原子函数,因子服务,因子函数

```
@[标准字段] =((CCustomContext *)(lpIUFTContext->GetBizContext()))->GetUsmsPartitionNo();
```

### 获取交易参数分片号

- **参数**: `[标准字段]`
- **适用类型**: 函数,服务,原子服务,原子函数,因子服务,因子函数

```
@[标准字段] =((CCustomContext *)(lpIUFTContext->GetBizContext()))->GetUargPartitionNo();
```

### 获取交易管理部署标志

- **参数**: `[标准字段]`
- **适用类型**: 函数,服务,原子服务,原子函数,因子服务,因子函数

```
@[标准字段] =((CCustomContext *)(lpIUFTContext->GetBizContext()))->GetUsmsDeployFlag();
```

### 获取交易参数部署标志

- **参数**: `[标准字段]`
- **适用类型**: 函数,服务,原子服务,原子函数,因子服务,因子函数

```
@[标准字段] =((CCustomContext *)(lpIUFTContext->GetBizContext()))->GetUargDeployFlag();
```

### 获取RPC调用USMS超时时间

- **参数**: `[标准字段]`
- **适用类型**: 函数,服务,原子服务,原子函数,因子服务,因子函数

```
@[标准字段] =((CCustomContext *)(lpIUFTContext->GetBizContext()))->GetUsmsRpcTimeout();
```

### 获取交易管理分片号

- **参数**: `[标准字段]`
- **适用类型**: 函数,服务,原子服务,原子函数,因子服务,因子函数

```
@[标准字段] =((CCustomContext *)(lpIUFTContext->GetBizContext()))->GetUsmsPartitionNo();
```

### 获取交易参数分片号

- **参数**: `[标准字段]`
- **适用类型**: 函数,服务,原子服务,原子函数,因子服务,因子函数

```
@[标准字段] =((CCustomContext *)(lpIUFTContext->GetBizContext()))->GetUargPartitionNo();
```

### 获取交易管理部署标志

- **参数**: `[标准字段]`
- **适用类型**: 函数,服务,原子服务,原子函数,因子服务,因子函数

```
@[标准字段] =((CCustomContext *)(lpIUFTContext->GetBizContext()))->GetUsmsDeployFlag();
```

### 获取交易参数部署标志

- **参数**: `[标准字段]`
- **适用类型**: 函数,服务,原子服务,原子函数,因子服务,因子函数

```
@[标准字段] =((CCustomContext *)(lpIUFTContext->GetBizContext()))->GetUargDeployFlag();
```

### 获取RPC调用USMS超时时间

- **参数**: `[标准字段]`
- **适用类型**: 函数,服务,原子服务,原子函数,因子服务,因子函数

```
@[标准字段] =((CCustomContext *)(lpIUFTContext->GetBizContext()))->GetUsmsRpcTimeout();
```

...
3. **阶段日志**：`[build-index] index_files_batch_committed` 会显示文件索引阶段已提交的文件数
4. **统计信息**：每个索引构建完成后显示文件数、过程数、表数量等统计
5. **完成提示**：显示所有构建的索引文件位置

大库构建时，代码索引的文件写入阶段会按批提交。默认每 `500` 个文件提交一次，所以 `.db-wal` 会增长，但主库不再需要等全部文件写完后才落盘。
